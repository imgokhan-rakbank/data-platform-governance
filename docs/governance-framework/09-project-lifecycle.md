...
from the approved Erwin model — no manual DDL authoring is permitted. The Model PR is raised only
after all artifacts are ready (Erwin model version-tagged + auto-generated DDL committed).
...
| 6 | Model PR raised after all artifacts ready; PR merged; CD pipeline has created / altered all physical entities in Databricks |
